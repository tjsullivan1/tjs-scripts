...
def parse_volume_info(
    volume,
    status,
    referral=None,
    priority="Low",
    rating=0,
    form_factor="Audiobook",
    date=None,
    category="Personal",
):
...
@click.option(
    ...
    default="to-read",
    type=click.Choice(
        ["to-read", "read", "not-to-read", "currently-reading"], case_sensitive=False
    ),
)
@click.option(
    "--date-read",
    default=None,
    help="When did you read the book? Formatted in MM/DD/YYYY.",
)
@click.option(
    "--referral",
    "--referred-by",
    help="Who told you about this or where did you find this reference?",
)
@click.option(
    "--result-size",
    "--results",
    "-r",
    default=1,
    help="How many book json objects do we want to return?",
)
@click.option(
    "--notion-key",
    "-nk",
    help="This is the integration key for Notion that allows writes to the database.",
)
@click.option(
    "--notion-database",
    "-ndb",
    help="This is the database ID that maps to your desired database.",
)
@click.option(
    "--no-notion",
    "-nn",
    is_flag=True,
    default=None,
    help="Use this flag to skip adding to Notion",
)
def main(
    title: str,
    status: str,
    ...
    for book in books:
        book_info = parse_volume_info(
            book.get("volumeInfo"), referral=referral, status=status, date=date_read
        )
        print(book_info)

        if no_notion is None:
            add_to_notion(
                book_info=book_info, database_id=notion_database, notion_key=notion_key
            )
